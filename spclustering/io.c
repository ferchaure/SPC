...
/* ----------------------------------------------------------------------- */
/* Read label files of prev temperature basename.th_01.lab, .dg_01.lab, and
update thOldBlock, dgOldBlock.
/* ----------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------- */
void PrintAverages(int nT, float T, double e, double ee, float nc,
                   float *Size) {
  int i, NS, MinSize;
  FILE *out;
...
void PrintMagnet(int nT, float T, float *M, float *xi) {
  ...
  fprintf(out, "%8.5f    ", T);
  for (i = 0; i < suscol; i++)
    fprintf(out, "%8.6f   ", M[i]);
  for (i = 0; i < suscol; i++)
    fprintf(out, "%8.6f   ", xi[i]);
  fprintf(out, "\n");
  fclose(out);
}
          ...
          for (k1 = 0; k1 < FPCorr.c[i1]; k1++) {
            if (NK.p[i1][k1] > i1) {
              dum = (((float)(FPCorr.p[i][k]).p[i1][k1]) -
                     ((float)Corr.p[i][k]) * ((float)Corr.p[i1][k1]) /
                         ((float)ncy)) /
                    ((float)ncy);
              dum = chkinteg(dum);
              s += J.p[i1][k1] * dum;
...
void ReadParamFromStr(char *param_str) {
  char s[PARAM_MAX_FIELD + 50], n[PARAM_MAX_FIELD + 1], ac;
  int l;
  s[0] = 0;
  ac = 0;
