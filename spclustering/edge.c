...
static void downheap_max(dindex *elements, size_t n, size_t pos) {
  ...
  if (((elements[pos].p - elements[left].p) >= 0) &&
      ((elements[pos].p - elements[right].p) >= 0))
    return;
...
void k_smallest_distance_index(dindex *arr, size_t k, size_t n, int **MNV,
                               size_t mnv_i) {
  ...
  // lookover elements with less distance in the original arr. If any you add it
...
void k_greatest_distance_index(dindex *arr, size_t k, size_t n, int **MNV,
                               size_t mnv_i) {
  ...
  for (i = 0; i < N; i++) {
    for (j = 0; j < K; j++) {
      ...
      for (k = 0; k < K && MNV[cand][k] != i; k++)
        ;
      MNV[i][j] -=
          (MNV[i][j] + 1) * (K == k); /* If the candidate is rejected */
...
u_0$. If $|V| = 1$ then stop, otherwise go to step 2.
...
void mstree(int N, int D, double **X, unsigned int **edg) {
  ...
  for (i = 0; i < (N - 1); i++) {
    ...
    for (j = 0; j < (N - i - 1); j++) {
      if (D)
        d = metric ? Squared_Distance(D, X[u], X[V[j]])
